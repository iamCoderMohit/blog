...
function EditBlog() {
  ...
  async function update() {
    try {
        setLoading(true)
        const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/blog/edit/${id}`, {title, content}, {withCredentials: true})
        setLoading(false)
    } catch (error) {
        console.error(error)
    }
    ...
    <MainTheme>
      ...
      >
        ...
        <div className="flex flex-col mt-5">
          ...
          <input type="text" className="outline-none border-b text-lg" value={loading ? "loading..." : title} 
          onChange={(e) => setTitle(e.target.value)}
          />
            <h1 className="text-2xl font-bold text-gray-500 mt-5">New Content</h1>
          <textarea name="" id="" className="text-lg outline-none" value={loading ? "loading..." : content}
          onChange={(e) => setContent(e.target.value)}
          ></textarea>
      <button className="bg-blue-700 rounded-md self-end px-5 mt-3 cursor-pointer"
      onClick={update}
